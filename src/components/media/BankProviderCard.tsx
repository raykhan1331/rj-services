...
export interface BankProvider {
  name: string;
  /** Which account type this provider is listed under. */
...
/**
 * Premium provider card. Deliberately text-based rather than a downloaded
 * bank logo: RJ Services has no confirmed licence to reproduce these
 * providers' trademarks, and Step 12 of the banking rules is explicit that
 * an unsafe logo must fall back to a clean text card rather than risk a
 * broken or unauthorised image. The initials badge is fully original.
 */
export default function BankProviderCard({ name, type, description, href }: BankProvider) {
  const initials = name
    .split(" ")
    .map((w) => w[0])
    .slice(0, 2)
